...
BOOL ConfigureBoard(HANDLE boardHandle)
{
	...
	retCode = 
		AlazarSetCaptureClock(
			...
			SAMPLE_RATE_800MSPS,	// U32 -- sample rate id
	...
	retCode = 
		AlazarInputControl(
			...
			INPUT_RANGE_PM_500_MV,	// U32 -- input range id
	...
	retCode = 
		AlazarInputControl(
			...
			DC_COUPLING,			// U32 -- input coupling id
			INPUT_RANGE_PM_20_MV,	// U32 -- input range id
			IMPEDANCE_50_OHM		// U32 -- input impedance id
			);
	if (retCode != ApiSuccess)
	{
	...
	U32 triggerTimeout_clocks = AutoTrigger ? 10U : 0U; // ns
